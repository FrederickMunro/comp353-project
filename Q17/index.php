...
WHERE firstName = :firstName AND lastName = :lastName AND nameOfFacility = :nameOfFacility;');
    ...
    if(!$appointment){
        ...
        $row = $nurseID->fetch(PDO::FETCH_ASSOC, PDO::FETCH_ORI_NEXT);
        ...
        $vaccineTime = date("Y-m-d H:i:s");

        $vaccination->bindParam(':publicHealthWorkerID', $row['employeeID']);
        ...
        $vaccination->bindParam(':dateOfVaccination', $vaccineTime);
...
<body>
    <?php if($ran == false && $appointment == true) { ?>
    <h1>Appointment Found! Can't Receive Vaccine</h1>
    <a href='../'>Return to menu</a>
    <?php } 
    else if($ran == false && $appointment == false){ ?>
        <h1>No Appointments Only!</h1>
